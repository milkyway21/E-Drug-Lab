#!/usr/bin/env python3
"""Reproducible 200 ns stability analysis for HSD17B13 Phase-B candidates.
...
DEFAULT_OUT = ROOT / "05_analysis/md200_selection"
INVENTORY = DEFAULT_OUT / "md_file_inventory.csv"
UPSTREAM_DECISION = ROOT / "05_analysis/md27_summary/md27_decision_table.csv"
...
def build_geometry_context(cms_model: Any, trajectory: Any) -> dict[str, Any]:
    protein_ca_aids = cms_model.select_atom("protein and atom.ptype CA")
    protein_heavy_aids = cms_model.select_atom("protein and not atom.ele H")
    ligand_all_aids = cms_model.select_atom("res.ptype UNK")
    ligand_heavy_aids = cms_model.select_atom("res.ptype UNK and not atom.ele H")
...
def main() -> None:
    global TRAJ_ROOT, SEA_ROOT, MANIFEST
    parser = argparse.ArgumentParser()
    parser.add_argument("--ids", nargs="*", default=None)
    parser.add_argument("--outdir", type=Path, default=DEFAULT_OUT)
    parser.add_argument("--no-structures", action="store_true")
    parser.add_argument("--trajectory-root", type=Path, default=TRAJ_ROOT)
    parser.add_argument("--sea-root", type=Path, default=SEA_ROOT)
    parser.add_argument("--manifest", type=Path, default=MANIFEST)
    args = parser.parse_args()
    TRAJ_ROOT = args.trajectory_root.resolve()
    SEA_ROOT = args.sea_root.resolve()
    MANIFEST = args.manifest.resolve()
    molecule_ids = args.ids or read_ids()
