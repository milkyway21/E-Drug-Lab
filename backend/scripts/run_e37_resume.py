#!/usr/bin/env python3
"""E37 Resume — 从已有 checkpoint 恢复，只跑未完成的部分。

已完成: E37b GRPO 训练, E37c Balanced 训练
待完成: E37a query, E37b/c query, E37j 训练+query, E37d-i 训练+query
"""
...
def train_if_needed(ckpt_path, label, train_fn):
    """如果 checkpoint 已存在则跳过训练，否则调用 train_fn。返回 (ok, ckpt_path)。"""
    if Path(ckpt_path).exists():
        print(f"  [{label}] ⏭️  Checkpoint exists ({Path(ckpt_path).stat().st_size/1e6:.0f}MB), skipping training")
        return True
    print(f"  [{label}] Training...")
    t0 = time.time()
    ok = train_fn()
    print(f"  [{label}] {'✅' if ok else '❌'} Trained in {time.time()-t0:.0f}s")
    return ok
...
def main():
    ...
    print("=" * 80)
    print("  E37 Resume — 从已有 checkpoint 恢复")
    print("=" * 80)
    ...
    # ═══════════════════════════════════════════════════════════
    # Phase 1: Query existing checkpoints (E37a, E37b, E37c)
    # ═══════════════════════════════════════════════════════════
    print(f"\n{'='*60}")
    print("  Phase 1: Query existing checkpoints (E37a/b/c)")
    print(f"{'='*60}")
    ...
    # E37b: GRPO (checkpoint exists, just query)
    print("\n--- E37b: GRPO from Scratch ---")
    ckpt_b = str(OUTPUT_DIR / 'e37b_grpo.pt')
    e37b_eval = rank_and_eval(ckpt_b, "E37b", r2_mols, POOL_CSV)
    all_schemes['E37b'] = {
    ...
    # E37c: Balanced (checkpoint exists, just query)
    print("\n--- E37c: Balanced Patent ---")
    ckpt_c = str(OUTPUT_DIR / 'e37c_balanced.pt')
    e37c_eval = rank_and_eval(ckpt_c, "E37c", r2_mols, POOL_CSV)
    all_schemes['E37c'] = {
    ...
    # ═══════════════════════════════════════════════════════════
    # Phase 2: E37j — High Pos Weight (need train + query)
    # ═══════════════════════════════════════════════════════════
    print(f"\n{'='*60}")
    print("  Phase 2: Group D — E37j High Pos Weight")
    print(f"{'='*60}")
    ...
    if not Path(ckpt_j).exists():
        ...
        print(f"  ⏭️  Checkpoint exists, querying...")
        e37j_eval = rank_and_eval(ckpt_j, "E37j", r2_mols, POOL_CSV)
    all_schemes['E37j'] = {
    ...
    # ═══════════════════════════════════════════════════════════
    # Phase 3: Fine-Tune from E34 with L2-SP (E37d/e/f/g/h/i)
    # ═══════════════════════════════════════════════════════════
    print(f"\n{'='*60}")
    print("  Phase 3: Group B+C — E34 Fine-Tune + L2-SP")
    print(f"{'='*60}")
    ...
    ft_schemes.append(('E37d', 'Pure L2-SP (13 only)', str(OUTPUT_DIR/'e37d_pure_l2sp.pt'),
                       dict(train_smiles=w13_smi[:], train_labels=w13_lab[:], sample_weights=w13_wgt[:],
                            prev_checkpoint=E34_CKPT, epochs=5, lr=3e-4, l2_lambda=1e-2,
                            ensemble_size=ENSEMBLE, strategy="supervised")))

    ft_schemes.append(('E37e', 'L2-SP + Decoys', str(OUTPUT_DIR/'e37e_l2sp_decoys.pt'),
                       dict(train_smiles=smi_e, train_labels=lab_e, sample_weights=wgt_e,
                            prev_checkpoint=E34_CKPT, epochs=5, lr=3e-4, l2_lambda=1e-2,
                            ensemble_size=ENSEMBLE, strategy="supervised")))

    ft_schemes.append(('E37f', 'L2-SP + Patent Anchor', str(OUTPUT_DIR/'e37f_l2sp_patent.pt'),
                       dict(train_smiles=w13_smi+pat100_smi+decoy_200,
                            train_labels=w13_lab+pat100_lab+[0]*len(decoy_200),
                            sample_weights=w13_wgt+pat100_wgt+[1.0]*len(decoy_200),
                            prev_checkpoint=E34_CKPT, epochs=5, lr=3e-4, l2_lambda=1e-2,
                            ensemble_size=ENSEMBLE, strategy="supervised")))

    ft_schemes.append(('E37g', 'L2-SP + Full 403 Patent', str(OUTPUT_DIR/'e37g_l2sp_fullpatent.pt'),
                       dict(train_smiles=w13_smi+p403_smi+decoy_200,
                            train_labels=w13_lab+p403_lab+[0]*len(decoy_200),
                            sample_weights=w13_wgt+p403_wgt+[1.0]*len(decoy_200),
                            prev_checkpoint=E34_CKPT, epochs=5, lr=3e-4, l2_lambda=1e-2,
                            ensemble_size=ENSEMBLE, strategy="supervised")))

    ft_schemes.append(('E37h', 'Weak L2-SP (l2=3e-3)', str(OUTPUT_DIR/'e37h_weak_l2sp.pt'),
                       dict(train_smiles=smi_e, train_labels=lab_e, sample_weights=wgt_e,
                            prev_checkpoint=E34_CKPT, epochs=5, lr=3e-4, l2_lambda=3e-3,
                            ensemble_size=ENSEMBLE, strategy="supervised")))

    ft_schemes.append(('E37i', 'Low LR (ep=10, lr=1e-4)', str(OUTPUT_DIR/'e37i_lowlr.pt'),
                       dict(train_smiles=smi_e, train_labels=lab_e, sample_weights=wgt_e,
                            prev_checkpoint=E34_CKPT, epochs=10, lr=1e-4, l2_lambda=1e-2,
                            ensemble_size=ENSEMBLE, strategy="supervised")))

    for key, label, ckpt_path, train_kwargs in ft_schemes:
        print(f"\n--- {key}: {label} ---")
        if not Path(ckpt_path).exists():
            t0 = time.time()
            r = train(ckpt_path, **train_kwargs)
            print(f"  Trained in {time.time()-t0:.0f}s, loss={r.get('final_loss')}")
            ev = rank_and_eval(ckpt_path, key, r2_mols, POOL_CSV) if r.get('ok') else None
        else:
            print(f"  ⏭️  Checkpoint exists, querying...")
            ev = rank_and_eval(ckpt_path, key, r2_mols, POOL_CSV)
        group = 'B' if key in ('E37d','E37e','E37f','E37g') else 'C'
        all_schemes[key] = {
            'label': label, 'checkpoint': ckpt_path, 'eval': ev,
            'paradigm': 'ft_l2sp', 'group': group,
        }
    ...
    print(f"\n  {'Scheme':>8s} {'Paradigm':>14s} {'Mean Rank':>12s} {'vs E34':>10s} {'Status':>8s}")
    print(f"  {'─'*60}")
    print(f"  {'E34':>8s} {'baseline':>14s} #{e34_mean:>11.0f} {'—':>10s} —")
    ...
    for key in sorted(all_schemes.keys()):
        ...
        if ev is None or ev['mean_rank'] is None:
            print(f"  {key:>8s} {paradigm:>14s} {'FAILED':>12s}")
            continue
        mean_r = ev['mean_rank']
        delta = e34_mean - mean_r
        better = '✅' if delta > 0 else '❌'
        print(f"  {key:>8s} {paradigm:>14s} #{mean_r:>11.0f} {delta:>+10.0f} {better:>8s}")
        if mean_r < best_mean:
            best_mean = mean_r
            best_scheme = key
